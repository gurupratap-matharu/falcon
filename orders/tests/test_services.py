...
from orders.services import order_confirmed, order_created
...
class OrderCreatedTests(TestCase):
    ...
    def test_order_creation_for_invalid_order_raises_404(self):
        ...
        with self.assertRaises(Http404):
            sent_mails = order_created(order_id=order_id)
            self.assertEqual(sent_mails, 0)
            self.assertEqual(len(mail.outbox), 0)

    def test_order_creation_sends_invoice_via_email(self):
